...
import com.andrew.hdss.data.dtos.IndividualDto
...
interface IndividualApiRepository {

    @GET("individuals")
    suspend fun getIndividuals(
        @QueryMap request: Map<String, String>
    ): Response<BatchResponse<IndividualDto>>
}

class IndividualApiService(
    private val individualApiRepository: IndividualApiRepository,
    ...
    suspend fun fetchIndividuals(
        ...
        while (true) {
            val response = try {
                individualApiRepository.getIndividuals(
    ...
    private fun <T> Response<T>.toFailureResult(): SyncResult {
        ...
        val errorResponse = rawBody?.let {
            try {
                ...
                Log.w("IndividualApiService", "Error body did not match ErrorResponse shape", e)
                null
            }
        }
